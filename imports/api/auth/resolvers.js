...
const mutations = {
  async loginWithPassword(_, { email, pwd }) {
    ...
    const authenticatingUser = await Meteor.users.findOne({
      'emails.address': email,
    });

    if (!authenticatingUser) throw 'UnAuthorized';
    if (
      !(authenticatingUser.services != null
        ? authenticatingUser.services.password
        : undefined)
    )
      throw 'Unauthorized';

    const passwordVerification = await Accounts._checkPassword(
      authenticatingUser,
      pwd
    );
    ...
    await Accounts._insertHashedLoginToken(authenticatingUser._id, {
